...
public abstract class MediaEncoder implements Runnable {
    ...
    protected volatile boolean shouldNotifyData = true;

    public MediaEncoder(Context context, final MediaMuxerWrapper muxer, final MediaEncoderListener listener) {
...
//            Log.e(TAG, "fuck:   flags=" + mBufferInfo.flags + "   dataLength:" + outData.length);
            if (mBufferInfo.flags == MediaCodec.BUFFER_FLAG_CODEC_CONFIG || outData[4] == 0x67) {
                isAdd = false;
//                Log.e(TAG, "SPS  PPS DATA");
                ...
                if (spsPpsBuffer.getInt() == 0x00000001) {
                    ...
                    //AccesssoryManager.getAccesssoryManager(mContext).sendData(mSpsAndPps);
                }
            } else if (mBufferInfo.flags == MediaCodec.BUFFER_FLAG_KEY_FRAME) {
                //关键帧

                isAdd = true;
//                Log.e(TAG, "Key Frame123");
                ...
                //Tony Send Start Stream Send H264
//				MainActivity.mainActivity.sendData(outData);
                AccesssoryManager.getAccesssoryManager(mContext).sendKeyFrame(data);
            } else {
//				Log.e(TAG, "========not Key Frame");
//				AccesssoryManager.getAccesssoryManager(mContext).showToast("1================");
                //非关键帧
                //Tony Send Start Stream Send H264
//				MainActivity.mainActivity.sendData(outData);
                if (!isAdd) {
...
//                    Log.e(TAG, "Key Frame");
                    ...
                    //Tony Send Start Stream Send H264
//				MainActivity.mainActivity.sendData(outData);
                    AccesssoryManager.getAccesssoryManager(mContext).sendNotKeyFrame(outData);
                }
            }
        }
    }
...
//                Log.d(TAG, "retrieving buffers time out!");
    ...
    void pauseScreenData() {
        shouldNotifyData = false;
    }

    void resumeScreenData() {
        shouldNotifyData = true;
    }
}
